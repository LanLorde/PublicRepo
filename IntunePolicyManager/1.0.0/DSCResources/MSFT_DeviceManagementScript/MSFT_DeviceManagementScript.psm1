...
function Test-TargetResource {
    ...
    $existing = Get-MgDeviceManagementScript | Where-Object { $_.DisplayName -eq $policy.displayName }
...
function Set-TargetResource {
    ...
    if ($existing) {
        ...
        Update-MgDeviceManagementScript -DeviceManagementScriptId $existing.Id -BodyParameter $policy
        ...
        [PSCustomObject]@{
            Timestamp    = $timestamp
            ResourceType = 'MSFT_DeviceManagementScript'
            DisplayName  = $policy.displayName
            Action       = 'Updated'
            Path         = $PolicyPath
        } | Export-Csv -Path $logFile -Append -NoTypeInformation -Encoding UTF8
    } else {
        New-MgDeviceManagementScript -BodyParameter $policy
